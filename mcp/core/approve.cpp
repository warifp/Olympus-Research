...
{
	m_type = WitnessElection;
	m_witnessElection = boost::make_optional<witness_election>(witness_election(_epoch, _proof));
	if (s)
		sign(s);
}
...
mcp::approve::approve(dev::RLP const & rlp, CheckTransaction _checkSig)
{
    try
	{
		...
		m_type = static_cast<ApproveType>(rlp[0].toInt<uint64_t>());

		if(WitnessElection == m_type){
			if (rlp.itemCount() != 6)
			BOOST_THROW_EXCEPTION(InvalidTransactionFormat() << errinfo_comment("too many or to low fields in the transaction RLP"));

			m_witnessElection = boost::make_optional<witness_election>(witness_election(rlp[1].toInt<uint64_t>(), (h648)rlp[2]));
...
Address const& mcp::approve::sender() const
{
	if (!m_sender.is_initialized())
	{
		...
		if(m_witnessElection)
		m_witnessElection->m_publicCompressed = dev::toPublicCompressed(p);
	}
	return *m_sender;
}
...
void mcp::approve::streamRLP(RLPStream& s, IncludeSignature sig) const
{
	if(m_witnessElection){
		s.appendList(6);
		s << m_type << m_witnessElection->m_epoch << m_witnessElection->m_proof;
...
{
	assert(m_type == WitnessElection);
	if (m_witnessElection->m_epoch != _epoch)
	{
		LOG(g_log.error) << "[checkEpoch] m_epoch = " << m_witnessElection->m_epoch << " _epoch = " <<_epoch;
		BOOST_THROW_EXCEPTION(InvalidSignature());
	}
}
...
void mcp::approve::vrf_verify(mcp::block_hash const& msg) const
{
	assert(m_type == WitnessElection);
	sender();
	if(!dev::verify(m_witnessElection->m_outputs, m_witnessElection->m_proof, m_witnessElection->m_publicCompressed, msg))
	{

...
void mcp::witness::check_and_witness()
{
	...
	mcp::witness_param const & w_param(mcp::param::get_witness_param(m_store, m_cache, mcp::approve::calc_curr_epoch(last_summary_mci + 1)));

	if (!mcp::param::is_witness(m_store, m_cache, mcp::approve::calc_curr_epoch(last_summary_mci + 1), m_account))
	{
		m_is_witnessing.clear();

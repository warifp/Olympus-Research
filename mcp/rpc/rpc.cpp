...
{
	mcp::witness_param const &w_param(mcp::param::get_witness_param(m_store, m_cache, m_chain->last_epoch()));
...
void mcp::rpc_handler::epoch_elected_approve_receipts(mcp::json &j_response, bool &)
{
	...
	if (epoch == 0)
	{
		for (auto witness : mcp::param::get_witness_param(m_store, m_cache, 0).witness_list)
		{
			mcp::json elected_l;
